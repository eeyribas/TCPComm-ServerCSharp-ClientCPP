...
void *TcpClientProcess(void *param);

pthread_t tcp_init_thread;
pthread_t tcp_receiver_thread;
pthread_t tcp_process_thread;
int client_socket;
int port_no = 5120;
...
int main(int argc, char *argv[])
{
    ...
    QNetworkInterface eth_0_ip = QNetworkInterface::interfaceFromName("eth0");
    QList<QNetworkAddressEntry> entries = eth_0_ip.addressEntries();
...
void *TcpClientReceiver(void *param)
{
    ...
    for (;;) {
        ...
        if (recv_count == recv_buffer_size) {
            ...
            if (recv_data[0] == 49) {
                process_state = true;
                if (pthread_create(&tcp_process_thread, nullptr, *TcpClientProcess, nullptr) == -1)
                    perror("Not create tcp process thread.\n");
            }
